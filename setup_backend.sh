...
check_env() {
  ...
  : "${TABLE_TOKENS_USURIOS:?Falta TABLE_TOKENS_USURIOS en .env}"
...
remove_infrastructure() {
  ...
  aws dynamodb delete-table --table-name "${TABLE_TOKENS_USURIOS}" 2>/dev/null || echo "Tabla ${TABLE_TOKENS_USURIOS} no existe"
